...
class _IsolatedObserver:
    """Pass-through Observer whose ``on_next`` is pre-wrapped by the bus.

    Installed by :meth:`ReactiveBus._safe_observable` at the outermost edge
    of a published observable. Everything except ``on_next`` is forwarded
    verbatim, so rx 3.2's ``AutoDetachObserver`` still sees the original
    handlers and keeps its own auto-dispose semantics intact — only the
    ``on_next`` call gains a try/except from the bus's ``_isolate``.

    This is a delegate rather than an ``Observer`` subclass so no private
    rx attribute (``is_stopped`` and friends) is shadowed: ``fail()`` and
    ``dispose()`` invoked on this proxy keep working.
    """

    __slots__ = ("_target", "_safe_on_next")

    def __init__(self, target: Any, isolate: Callable[[Any], Any]) -> None:
        self._target = target
        # Wrapped once at subscribe time, not per message: this sits on the
        # hot delivery path and the bus is throughput-benchmarked.
        self._safe_on_next = isolate(self._deliver)

    def _deliver(self, value: Any) -> None:
        self._target.on_next(value)

    def on_next(self, value: Any) -> None:
        self._safe_on_next(value)

    def on_error(self, error: Any) -> None:
        self._target.on_error(error)

    def on_completed(self) -> None:
        self._target.on_completed()

    def fail(self, exn: Any) -> bool:
        return self._target.fail(exn)
...
class ReactiveBus:
    ...
    def of_type(self, msg_type: type) -> rx.Observable:
        ...
        CRITICAL: the returned Observable is built with :meth:`_safe_observable`
        so that EVERY subscriber's ``on_next`` is routed through ``_isolate``,
        exactly like :meth:`subscribe`. Callers subscribe by calling
        ``.subscribe()`` on the returned Observable, bypassing
        :meth:`subscribe` entirely, and rx 3.2's ``AutoDetachObserver.on_next``
        does NOT catch exceptions. Without this wrap, one raising ``of_type``
        handler escapes ``publish()`` and is never disposed, permanently
        starving every sibling observer on the same Subject. The engine
        (``engine.py:285/295/304``) and the durable fill store
        (``sqlite_store.py:587``) both subscribe this way.
        """
        lane = _lane_for(msg_type)
        subject = self._lane_subjects.get(lane)
        if subject is not None and lane != _LANE_DEFAULT:
            return self._safe_observable(
                subject.pipe(ops.filter(lambda m: isinstance(m, msg_type))),
            )
        # Default lane or unknown type: use the all-lane Subject so
        # subscribers to non-domain types still work.
        return self._safe_observable(
            self._subject.pipe(ops.filter(lambda m: isinstance(m, msg_type))),
        )

    def stream(self) -> rx.Observable:
        """Raw Observable of ALL messages."""
        return self._safe_observable(self._subject)

    def _safe_observable(self, source: rx.Observable) -> rx.Observable:
        """Wrap *source* so that every subscriber is error-isolated.

        Isolation is applied by wrapping the *inbound observer* rather than
        the source chain: ``_subscribe_core`` hands downstream a
        :class:`_IsolatedObserver` whose ``on_next`` is already
        ``_isolate``-wrapped. This is entry-point agnostic, so the callback
        form (``obs.subscribe(fn)``), the Observer form
        (``obs.subscribe(observer)``) and any operator pipelined via
        ``obs.pipe(...)`` are all protected, and nothing upstream can raise
        out of ``publish()``.

        The lane/type filter stays *inside* ``source``, and the operator
        chain is not re-``share()``d, so a second ``of_type(X)`` subscriber
        cannot have its handler mangled by the first one's isolation.
        """
        isolate = self._isolate

        def subscribe_core(observer: Any, scheduler: Any = None) -> Any:
            # Dispatch through the *source's own* _subscribe_core: Subject
            # overrides it to register the observer, and the Observable
            # base-class version would silently register nothing.
            return source._subscribe_core(  # noqa: SLF001 – rx has no public hook
                _IsolatedObserver(observer, isolate), scheduler
            )

        return rx.core.Observable(subscribe_core)
