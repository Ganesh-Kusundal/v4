...
# ---------------------------------------------------------------------------
# Extended endpoints: alerts, triggers, pnl exit, ip, options
# ---------------------------------------------------------------------------


class TestExtendedEndpoints:
    """Thin pass-throughs to the client for the extended Dhan endpoints."""

    def test_place_alert(self):
        broker, transport = _make_broker()
        transport.place_alert.return_value = {"alertId": "A1"}
        result = broker.place_alert(price=100.0)
        assert result["alertId"] == "A1"
        transport.place_alert.assert_called_once_with(price=100.0)

    def test_get_alert(self):
        broker, transport = _make_broker()
        transport.get_alert.return_value = {"alertId": "A1"}
        assert broker.get_alert("A1")["alertId"] == "A1"
        transport.get_alert.assert_called_once_with("A1")

    def test_list_alerts(self):
        broker, transport = _make_broker()
        transport.list_alerts.return_value = [{"alertId": "A1"}]
        assert broker.list_alerts() == [{"alertId": "A1"}]

    def test_delete_alert(self):
        broker, transport = _make_broker()
        transport.delete_alert.return_value = {"status": "DELETED"}
        assert broker.delete_alert("A1")["status"] == "DELETED"
        transport.delete_alert.assert_called_once_with("A1")

    def test_place_conditional_trigger(self):
        broker, transport = _make_broker()
        transport.place_conditional_trigger.return_value = {"triggerId": "CT1"}
        result = broker.place_conditional_trigger(price=100.0)
        assert result["triggerId"] == "CT1"
        transport.place_conditional_trigger.assert_called_once_with(price=100.0)

    def test_get_conditional_trigger(self):
        broker, transport = _make_broker()
        transport.get_conditional_trigger.return_value = {"triggerId": "CT1"}
        assert broker.get_conditional_trigger("CT1")["triggerId"] == "CT1"

    def test_list_conditional_triggers(self):
        broker, transport = _make_broker()
        transport.list_conditional_triggers.return_value = [{"triggerId": "CT1"}]
        assert broker.list_conditional_triggers() == [{"triggerId": "CT1"}]

    def test_delete_conditional_trigger(self):
        broker, transport = _make_broker()
        transport.delete_conditional_trigger.return_value = {"status": "DELETED"}
        assert broker.delete_conditional_trigger("CT1")["status"] == "DELETED"

    def test_modify_conditional_trigger(self):
        broker, transport = _make_broker()
        transport.modify_conditional_trigger.return_value = {"triggerId": "CT1"}
        result = broker.modify_conditional_trigger("CT1", validity="2026-12-31")
        assert result["triggerId"] == "CT1"
        transport.modify_conditional_trigger.assert_called_once_with(
            "CT1", validity="2026-12-31"
        )

    def test_configure_pnl_exit(self):
        broker, transport = _make_broker()
        transport.configure_pnl_exit.return_value = {"configured": True}
        assert broker.configure_pnl_exit(pnlPercent=5.0)["configured"] is True
        transport.configure_pnl_exit.assert_called_once_with(pnlPercent=5.0)

    def test_stop_pnl_exit(self):
        broker, transport = _make_broker()
        transport.stop_pnl_exit.return_value = {"stopped": True}
        assert broker.stop_pnl_exit()["stopped"] is True
        transport.stop_pnl_exit.assert_called_once_with()

    def test_get_pnl_exit(self):
        broker, transport = _make_broker()
        transport.get_pnl_exit.return_value = {"pnlPercent": 5.0}
        assert broker.get_pnl_exit()["pnlPercent"] == 5.0

    def test_set_ip(self):
        broker, transport = _make_broker()
        transport.set_ip.return_value = {"status": "SET"}
        assert broker.set_ip(ipAddress="1.2.3.4")["status"] == "SET"
        transport.set_ip.assert_called_once_with(ipAddress="1.2.3.4")

    def test_modify_ip(self):
        broker, transport = _make_broker()
        transport.modify_ip.return_value = {"status": "MODIFIED"}
        assert broker.modify_ip(ipAddress="1.2.3.4")["status"] == "MODIFIED"

    def test_get_ip(self):
        broker, transport = _make_broker()
        transport.get_ip.return_value = {"ipAddress": "1.2.3.4"}
        assert broker.get_ip()["ipAddress"] == "1.2.3.4"

    def test_get_expired_option_data(self):
        broker, transport = _make_broker()
        transport.get_expired_option_data.return_value = {"open": [1.0]}
        result = broker.get_expired_option_data(_equity())
        assert result["open"] == [1.0]
        transport.get_expired_option_data.assert_called_once_with(
            _equity(), "1d", None, None
        )

    def test_expiry_list(self):
        broker, transport = _make_broker()
        transport.expiry_list.return_value = ["2026-08-27"]
        assert broker.expiry_list(_index()) == ["2026-08-27"]
        transport.expiry_list.assert_called_once_with(_index())

    def test_get_rolling_options(self):
        broker, transport = _make_broker()
        transport.get_rolling_options.return_value = {"ce": {"open": [1.0]}}
        result = broker.get_rolling_options(
            _index(),
            expiry_flag="WEEK",
            expiry_code=0,
            strike="ATM",
        ...
        kwargs = transport.get_rolling_options.call_args.kwargs
        assert kwargs["expiry_flag"] == "WEEK"
        assert kwargs["expiry_code"] == 0
        assert kwargs["required_data"] is None
        assert kwargs["interval"] == 1

    def test_place_alert_respects_mutation_gate(self):
        transport = MagicMock(spec=DhanApiClient)
        broker = DhanBroker(transport=transport, allow_order_operations=False)
        broker.connect()
        with pytest.raises(OrderRejectedError):
            broker.place_alert(price=100.0)
