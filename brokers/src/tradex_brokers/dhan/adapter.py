...
class DhanBroker(BaseBroker):
    ...
    def place_alert(self, **params: object) -> dict[str, object]:
        self._require_mutation()
        return self._require().place_alert(**params)

    def get_alert(self, alert_id: str) -> dict[str, object]:
        return self._require().get_alert(alert_id)

    def list_alerts(self) -> list[dict[str, object]]:
        return list(self._require().list_alerts())

    def delete_alert(self, alert_id: str) -> dict[str, object]:
        self._require_mutation()
        return self._require().delete_alert(alert_id)

    def place_conditional_trigger(self, **params: object) -> dict[str, object]:
        self._require_mutation()
        return self._require().place_conditional_trigger(**params)

    def get_conditional_trigger(self, alert_id: str) -> dict[str, object]:
        return self._require().get_conditional_trigger(alert_id)

    def list_conditional_triggers(self) -> list[dict[str, object]]:
        return list(self._require().list_conditional_triggers())

    def delete_conditional_trigger(self, alert_id: str) -> dict[str, object]:
        self._require_mutation()
        return self._require().delete_conditional_trigger(alert_id)

    def modify_conditional_trigger(
        self, alert_id: str, **params: object
    ) -> dict[str, object]:
        self._require_mutation()
        return self._require().modify_conditional_trigger(alert_id, **params)

    def configure_pnl_exit(self, **params: object) -> dict[str, object]:
        self._require_mutation()
        return self._require().configure_pnl_exit(**params)

    def stop_pnl_exit(self) -> dict[str, object]:
        self._require_mutation()
        return self._require().stop_pnl_exit()

    def get_pnl_exit(self) -> dict[str, object]:
        return self._require().get_pnl_exit()

    def set_ip(self, **params: object) -> dict[str, object]:
        self._require_mutation()
        return self._require().set_ip(**params)

    def modify_ip(self, **params: object) -> dict[str, object]:
        self._require_mutation()
        return self._require().modify_ip(**params)

    def get_ip(self) -> dict[str, object]:
        return self._require().get_ip()
    ...
    def expiry_list(self, instrument: Instrument) -> list[str]:
        return list(self._require().expiry_list(instrument))
    ...
    def get_rolling_options(
        ...
        *,
        ...
        return self._require().get_rolling_options(
            instrument,
            ...
            from_date=from_date,
            to_date=to_date,
            required_data=list(required_data) if required_data is not None else None,
            interval=interval,
        )
