...
from tradex_analytics.indicators import ema, roc, rsi, sma
...
class AnalyticsEngine:
    ...
    def indicator_values(
        ...
        **params: Any,
        ...
        if self._warmup_bars > 0:
            ...
            padded.extend(float(v) for v in raw_values if v is not None)
            return padded
        return [float(v) if v is not None else None for v in raw_values]
            ...
            return {
                "realized_vol": realized_vol(
                    ...
                    periods_per_year=int(
                        params.get("periods_per_year", TRADING_DAYS_PER_YEAR)
                    ),
                )
            }
        if key == "win_rate":
    ...
    def tearsheet(self, returns: Sequence[float] | None = None) -> dict[str, Any]:
        ...
        vol = math.sqrt(var_r * TRADING_DAYS_PER_YEAR)  # annualized

        # Sortino (downside deviation)
        downside = [r for r in ret_list if r < 0]
        if downside:
            downside_var = sum(r**2 for r in downside) / len(downside)
            downside_dev = math.sqrt(downside_var * TRADING_DAYS_PER_YEAR)
            sortino = (
                (mean_r * TRADING_DAYS_PER_YEAR) / downside_dev
                if downside_dev > 0
                else 0
            )
        else:
        ...
        # Calmar
        calmar = (mean_r * TRADING_DAYS_PER_YEAR) / abs(md) if md != 0 else 0.0
